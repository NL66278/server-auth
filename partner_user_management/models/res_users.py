...
class Resusers(models.Model):
    ...
    @api.model
    def create_for_partner(self, partner, template_user=False, login=False):
        ...
        if partner.user_ids:
            raise exceptions.UserError(
                _("Partner %s already has a user account") % partner.display_name
            )
        ...
        vals = {"partner_id": partner.id, "login": login, "active": True}
        if template_user:
    ...
    def get_template_user(self, partner):
        ...
        """
        return (
            self.env.ref("auth_signup.default_template_user", raise_if_not_found=False)
            or self.env.ref("base.default_user", raise_if_not_found=False)
            or False
        )
